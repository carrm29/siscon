...
class AgregarAjax(CreateView, TemplateView):
    def post(self, request, *args, **kwargs):
        ...
        # print notas
        data = serializers.serialize('json', datos, fields=('fecha', 'titulo', 'nota'))
        # print data
...
class Menu(ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated():
            ...
            return HttpResponseRedirect('/') # Si no estoy logueado me irecciona al login
...
class BuscarGananciasGastos(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        if palabra == '':
            ...
            data = serializers.serialize('json', datos, fields=('fecha', 'ganancia', 'gasto', 'nota'))
            # print data
            return HttpResponse(data)
        else:
            busqueda = Registro.objects.filter(usuario__id=id_user, fecha=palabra)
            data = serializers.serialize('json', busqueda, fields=('fecha', 'ganancia', 'gasto', 'nota'))
            # print data
            return HttpResponse(data)
...
class BuscarNotas(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        if palabra == '':
            ...
            busqueda = Notas.objects.filter(Q(titulo__contains=palabra) | Q(nota__contains=palabra),
                                            usuario__id=id_user)

            data = serializers.serialize('json', busqueda, fields=('fecha', 'titulo', 'nota'))
            return HttpResponse(data)
