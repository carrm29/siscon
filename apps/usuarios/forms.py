...
#class UserClave(forms.Form):
class UserClave(forms.Form):
    correo = forms.EmailField(
        required=True,
        max_length=50,
        widget=forms.TextInput(
            attrs={'class': 'form-control', 'type': 'email', 'placeholder': 'example@gmail.com'}))
    ...
    def clean_email(self):
        ...
        #    User._default_manager.get(email=correo)
        #except User.DoesNotExist:
        #   raise forms.ValidationError('El correo no ha sido registrado')
        return correo

    #modificamos el usuario desactivado
    #def save(self, commit=True):
    #    user = super(UserClave, self)
...
class EditarContrasenaMForm(forms.Form):
